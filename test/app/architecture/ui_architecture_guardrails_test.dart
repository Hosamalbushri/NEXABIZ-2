...
void main() {
  group('UI Architecture Guardrails (UI-01..04)', () {
    ...
    test(
      'UI-01 & UI-03: Presentation files MUST NOT directly import shadcn_flutter',
      () {
        final files = getDartFiles();
        for (final file in files) {
          final content = file.readAsStringSync();
          final containsShadcnImport =
    ...
    test(
      'UI-02: Presentation screens MUST NOT directly import flutter/material.dart for UI primitives',
      () {
        final files = getDartFiles();
        for (final file in files) {
          final content = file.readAsStringSync();
          final containsMaterialImport =
    ...
    test('UI-04: Reusable visual components live in packages/nexabiz_ui', () {
      final nexabizUiWidgetsDir = Directory(
        'packages/nexabiz_ui/lib/src/widgets',
      );
      expect(
