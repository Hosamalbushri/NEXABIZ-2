...
import 'package:nexabiz/core/session/core_session_listenable.dart';
...
class _MockIdentityStore implements CoreIdentityQueryStore {
  @override
  Future<CoreAuthUserRef?> findUserByIdentifier(String normalizedIdentifier) async => null;

  @override
  Future<CorePreparedCredential?> readUserCredential(String userId) async => null;

  @override
  Future<CoreAuthIdentitySnapshot?> readAuthenticationSnapshot(String userId) async => null;

  @override
  Future<CoreLoginLockout?> checkLockout(String normalizedIdentifier, DateTime nowUtc) async => null;

  @override
  Future<CoreLoginLockout?> recordFailedAttempt(String normalizedIdentifier, DateTime nowUtc) async => null;

  @override
  Future<void> clearFailedAttempts(String normalizedIdentifier) async {}

  @override
  Future<bool> isSessionEligible(String userId, String? companyId) async => true;
...
class _RouterTestNavContribution implements NexaBizNavigationContribution {
  ...
  List<NexaBizRouteDefinition> get routes => [
        ...
        _route('login', '/login', const NexaBizRouteAccessRequirement(requiresReadySetup: true)),
        _route('company_selection', '/company-selection', const NexaBizRouteAccessRequirement(requiresReadySetup: true, requiresActiveSession: true)),
        _route('dashboard', '/dashboard', const NexaBizRouteAccessRequirement(requiresReadySetup: true, requiresActiveSession: true, requiresCompanyScope: true)),
        _route('services', '/services', const NexaBizRouteAccessRequirement(requiresReadySetup: true, requiresActiveSession: true, requiresCompanyScope: true)),
        _route('reports', '/reports', const NexaBizRouteAccessRequirement(requiresReadySetup: true, requiresActiveSession: true, requiresCompanyScope: true)),
        _route('settings', '/settings', const NexaBizRouteAccessRequirement(requiresReadySetup: true, requiresActiveSession: true, requiresCompanyScope: true)),
        _route('unauthorized', '/unauthorized', const NexaBizRouteAccessRequirement()),
        _route(
          'billing_protected',
          '/billing',
  ...
  group('Router Authorization Integration & Defense-in-Depth Tests', () {
    ...
    setUp(() {
      ...
      sessionController = CoreSessionController(
        authenticateLocalUser: AuthenticateLocalUser(queryStore: _MockIdentityStore()),
    ...
    GoRouter buildRouter({
      ...
      return adapter.createRouter(
        ...
        permissionEvaluator: omitEvaluator ? null : (customEvaluator ?? evaluator),
    ...
    testWidgets('2. Session-protected route still redirects unauthenticated -> /login', (tester) async {
    ...
    testWidgets('3. Company-scoped route still redirects missing company -> /company-selection', (tester) async {
      sessionController.setSessionForTesting(
        NexaBizSession.active(
    ...
    testWidgets('4. Permission route + explicit grant -> accessible', (tester) async {
    ...
    testWidgets('5. Permission route + missing grant -> access denied (/unauthorized)', (tester) async {
      setControllerActiveSession();
    ...
    testWidgets('6. Permission route + unknown permission -> access denied (/unauthorized)', (tester) async {
      setControllerActiveSession();
    ...
    testWidgets('7. Permission route + missing evaluator -> access denied (/unauthorized)', (tester) async {
      setControllerActiveSession();
    ...
    testWidgets('8. Permission route + evaluator failure -> access denied (/unauthorized)', (tester) async {
      setControllerActiveSession();
    ...
    testWidgets('9. Stale session context -> access denied (/unauthorized)', (tester) async {
      setControllerActiveSession(sessionId: 'session-new');
      sessionSource.activeSessionId = 'session-stale'; // Evaluator session source mismatch
    ...
    testWidgets('10. Wrong company context -> access denied (/unauthorized)', (tester) async {
      setControllerActiveSession(companyId: NexaBizCompanyId('different-company'));
    ...
    testWidgets("11. Role name 'owner' without grant -> access denied (/unauthorized)", (tester) async {
    ...
    testWidgets("12. Role name 'admin' without grant -> access denied (/unauthorized)", (tester) async {
    ...
    testWidgets('13. Permission grant during active session -> accessible after reevaluation', (tester) async {
      setControllerActiveSession();
    ...
    testWidgets('14. Permission revoke during active session -> current protected route redirects to /unauthorized', (tester) async {
      setControllerActiveSession();
      queryStore.snapshot = _createSnapshot(permissions: {_permProtected});
    ...
    testWidgets('15. Company switch invalidates old route authorization', (tester) async {
      setControllerActiveSession();
      queryStore.snapshot = _createSnapshot(permissions: {_permProtected});
    ...
    testWidgets('16. Logout invalidates protected route -> redirects to /login', (tester) async {
      setControllerActiveSession();
      queryStore.snapshot = _createSnapshot(permissions: {_permProtected});
    ...
    testWidgets('18. Authorization denial does not redirect to login', (tester) async {
      setControllerActiveSession();
      queryStore.snapshot = _createSnapshot(permissions: {});
    ...
    testWidgets('19. Authorization denial does not redirect to company-selection', (tester) async {
      setControllerActiveSession();
      queryStore.snapshot = _createSnapshot(permissions: {});
    ...
    testWidgets('21. Concurrency race: logout or company switch during pending evaluation rejects stale allow', (tester) async {
      setControllerActiveSession();
    ...
    test('22. Defense-in-depth: UseCase PermissionGuard remains authoritative', () async {
