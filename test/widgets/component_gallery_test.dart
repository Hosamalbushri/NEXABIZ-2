...
void main() {
  testWidgets(
    'ComponentGalleryPage renders header and category filters correctly',
    (tester) async {
      await tester.pumpWidget(
        const shadcn.ShadcnApp(home: ComponentGalleryPage()),
      );
      await tester.pumpAndSettle();

      expect(
        find.text('Shadcn Flutter Component Gallery & Playground'),
        findsOneWidget,
      );
  ...
  testWidgets(
    'GalleryStateController updates search query and category filters',
    (tester) async {
      ...
      expect(
        controller.isComponentMatching(
          'Button',
          'Action trigger',
  ...
  testWidgets(
    'GalleryStateController toggles directionality and viewport size',
    (tester) async {
      final controller = GalleryStateController();
  ...
  test(
    'GalleryCapability registers /gallery route in NexaBizCapabilityManifest',
    () {
      ...
      final galleryCap = capabilities.firstWhere(
        (c) => c.capabilityId == 'gallery',
      );
      expect(
        galleryCap.navigationContribution?.rootRouteId,
        equals(const NexaBizRouteId(namespace: 'gallery', routeName: 'root')),
      );
      expect(
        galleryCap.navigationContribution?.routes.first.path,
        equals('/gallery'),
      );
    },
  );
