...
void main() {
  group('Router Authorization Defense-in-Depth Architecture Guardrails', () {
    ...
    test(
      'Router MUST NOT directly reference CoreAuthorizationQueryStore or database query stores',
      () {
        ...
        for (final file in files) {
          ...
          for (final ref in forbiddenStoreReferences) {
            expect(
              ...
              isFalse,
              reason:
                  '${file.path} must not reference $ref directly. Router must only use NexaBizPermissionEvaluator abstraction.',
            );
          }
        }
      },
    ...
    test(
      'Router MUST NOT inspect hardcoded role names or bypass permission evaluator via role checks',
      () {
        ...
        for (final pattern in forbiddenRolePatterns) {
          expect(
            ...
            isFalse,
            reason:
                'Router must NOT perform role-based bypasses (matched ${pattern.pattern}). Access decisions must be capability-permission based.',
          );
        }
      },
    );
    ...
    test(
      'Router MUST NOT contain secondary/hardcoded path-to-permission mapping table',
      () {
        final routerFile = File('lib/app/router/nexabiz_router_adapter.dart');
        expect(routerFile.existsSync(), isTrue);
        ...
        for (final pattern in forbiddenMapPatterns) {
          expect(
            ...
            isFalse,
            reason:
                'Router must get permissions strictly from NexaBizRouteDefinition.accessRequirement.permission, not an ad-hoc map.',
          );
        }
      },
    );
    ...
    test(
      'Unauthorized route (/unauthorized) MUST exist in Permissions capability and be accessible without permissions',
      () {
        ...
        expect(
          ...
          isTrue,
          reason:
              'Permissions capability must contribute the canonical /unauthorized route.',
        );
        expect(
          content.contains('requiresReadySetup: false') ||
              !content.contains(
                "path: '/unauthorized',\n          accessRequirement: NexaBizRouteAccessRequirement(permission:",
              ),
          isTrue,
          reason:
              '/unauthorized must not require permission or cause redirect loops.',
        );
      },
    );

    test(
      'Router redirects access denials to canonical /unauthorized and never leaks to /login or /company-selection',
      () {
        final routerFile = File('lib/app/router/nexabiz_router_adapter.dart');
        final content = routerFile.readAsStringSync();
        ...
        expect(
          ...
          isTrue,
          reason:
              'Router must redirect authorization failures to /unauthorized.',
        );
      },
    );

    test(
      'Router invalidation signal MUST decouple Router from persistence tables',
      () {
        final signalFile = File(
          'lib/app/authorization/nexabiz_authorization_invalidation_signal.dart',
        );
        ...
        expect(
          ...
          isFalse,
          reason:
              'Authorization invalidation signal must be persistence-agnostic.',
        );
        expect(
