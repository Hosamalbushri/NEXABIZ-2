...
void main() {
  group('Core Authorization & Roles Architecture Guardrail Tests', () {
    test('authorization and roles domain contracts MUST remain pure Dart and framework-neutral', () {
      final authDir = Directory('lib/core/authorization');
      final rolesDir = Directory('lib/core/roles');

      final files = <File>[
        if (authDir.existsSync()) ...authDir.listSync().whereType<File>(),
        if (rolesDir.existsSync()) ...rolesDir.listSync().whereType<File>(),
      ].where((f) => f.path.endsWith('.dart')).toList();

      expect(files, isNotEmpty, reason: 'Authorization and roles contract files must exist.');
      ...
      for (final file in files) {
        ...
        for (final match in matches) {
          ...
          expect(
            uri.contains('/app/'),
            isFalse,
            reason: '${file.path} must not import app infrastructure code ($uri)',
          );
        }
      }
    });
