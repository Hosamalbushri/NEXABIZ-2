...
void main() {
  group('Guardrail 10, 11 & 12 — Token, Breakpoint & RTL Guardrails', () {
    test(
      'Rule 10: Feature UI MUST NOT use hardcoded Color(0xFF...) literals',
      () {
    ...
    test(
      'Rule 11: Feature code MUST use canonical AppBreakpoints instead of arbitrary MediaQuery width checks',
      () {
        final appProductionFiles = _getAppProductionDartFiles();
        final breakpointRegex = RegExp(
          r'width\s*(?:<|<=|>|>=)\s*\d+|MediaQuery\.of\(context\)\.size\.width',
        );
        final violatingFiles = <String>[];
    ...
    test(
      'Rule 12: Production UI MUST use directional geometry (EdgeInsetsDirectional, AlignmentDirectional) where applicable',
      () {
        final appProductionFiles = _getAppProductionDartFiles();
        final rtlRegexes = [
