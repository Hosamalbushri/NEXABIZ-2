...
/// Application UseCase for unassigning a role from a membership.
///
...
final class UnassignRoleFromMembershipUseCase {
  const UnassignRoleFromMembershipUseCase({
  ...
  execute({
    ...
    // 3. Transactional Store Mutation (enforces last-active-owner invariant atomically)
    final result = await _store.unassignRoleFromMembership(
