...
void main() {
  group('Design System Boundary & Token Authority Architecture Tests', () {
    test('Rule A: Application lib directory contains zero package:nexabiz_ui/src/ imports', () {
      ...
      if (!appLibDir.existsSync()) {
        ...
        expect(rootAppLibDir.existsSync(), isTrue,
    ...
    test('Rule B: Canonical Design Token APIs are publicly exported from nexabiz_ui', () {
    ...
    test('Rule D: AppBreakpoints resolves to canonical layout implementation', () {
      ...
      expect(AppBreakpoints.getTier(1100), equals(AppBreakpointTier.expanded));
      expect(AppBreakpoints.getTier(1500), equals(AppBreakpointTier.wide));
    });

    test('Rule E: Phase 03-C Canonical Page Architecture APIs remain publicly exported', () {
...
void _assertNoInternalImports(Directory dir) {
  ...
  expect(violations, isEmpty,
      reason: 'Found direct internal package:nexabiz_ui/src/ imports in app feature code:\n'
          '${violations.join('\n')}');
}
